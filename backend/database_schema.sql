...
CREATE INDEX idx_files_classification ON files(matiere, sous_matiere, enseignant, semestre, promo);
CREATE INDEX idx_files_processed ON files(is_processed);
CREATE INDEX idx_chunks_file_id ON rag_chunks(file_id);
CREATE INDEX idx_chunks_classification ON rag_chunks(matiere, sous_matiere, enseignant, semestre, promo);
...
GROUP BY f.id, f.filename, f.matiere, f.sous_matiere, f.enseignant, f.semestre, f.promo, f.is_processed, f.chunk_count, f.upload_date;
